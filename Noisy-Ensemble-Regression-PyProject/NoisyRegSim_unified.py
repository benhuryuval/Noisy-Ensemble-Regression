...
data_type_vec = ["sin", "exp", "make_reg", "diabetes", "white-wine", "kc_house_data"]  # kc_house_data / diabetes / white-wine / sin / exp / make_reg
data_type_vec = ["exp"]
                ...
                for _m_idx, _m in enumerate(ensemble_size):
                        ...
                        for train_index, test_index in kf.split(X):
                                ...
                                for idx_snr_db, snr_db in enumerate(snr_db_vec):
                                        ...
                                        print("\t\t\t\tError reduction [%], (Non-robust-Robust)/Clean(r) = " +
                                              "{0:0.3f}".format( 100 * (err_nr[idx_snr_db, _m_idx, kfold_idx] - err_r[idx_snr_db, _m_idx, kfold_idx]) /
                                                                 np.min([err_cln[idx_snr_db, _m_idx, kfold_idx], err_rcln[idx_snr_db, _m_idx, kfold_idx]] ) )
                                              )
                                        if criterion == "mae":
                        ...
                        if plot_flag:
                                ...
                                for _m_idx, _m in enumerate(ensemble_size):
                                        ...
                                        gain = (err_nr[:, _m_idx, :].mean(1) - err_r[:, _m_idx, :].mean(1)) / ref_err
                                        deg = (err_r[:, _m_idx, :].mean(1) - ref_err) / ref_err
                                        plt.plot(snr_db_vec, gain, '-ob', snr_db_vec, deg, '-xb')
