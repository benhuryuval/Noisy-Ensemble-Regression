...
results_folder_path = os.path.join("Results", "2024_02_20")
...
plt.ylabel(criterion.upper()+' Reduction [dB]', fontsize=18)
...
for data_type_idx, data_type in enumerate(data_type_vec):
    ...
    err_reduction_db_vec = err_results_df[reg_algo + ', Non-Robust'] - err_results_df[reg_algo + ', Robust']
    if criterion == "mae":
        err_reduction_db_vec = 2*err_reduction_db_vec

    # color = next(ax._get_lines.prop_cycler)['color']
    # plt.plot(snr_db_vec, err_reduction_db_vec,
    ...
    plt.plot(snr_db_vec, 10**(err_results_df[reg_algo+', Non-Robust']/10), '-xr', label='Non-robust')
    plt.plot(snr_db_vec, 10**(err_results_df[reg_algo+', Robust']/10), '-ob', label='Robust')
